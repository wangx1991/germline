...
def umitool(samtools_dir, umitools_dir, filtered_sam ,filtered_bam , sorted_bam, umitool_stats , umis_sam, edit_dist, logger_umi_process, logger_umi_errors):
    ...
    logger_umi_process.info('Samtools transform sam to bem.')
    os.system(cmd1)
    cmd2 = samtools_dir + ' sort ' + filtered_bam + ' > ' + sorted_bam
    logger_umi_process.info('Samtools sort bem.')
    os.system(cmd2)
    cmd3 = samtools_dir + ' index ' + sorted_bam
    logger_umi_process.info('Samtools build index of bem.')
    os.system(cmd3)
    ...
    logger_umi_process.info('UMIs-tools cluster bem.')
    os.system(cmd4)
